...
describe("toPoints", () => {
  it("keeps only rows where weightKg is non-null (sparse)", () => {
  ...
  it("returns an empty array when no row has weightKg logged", () => {
...
describe("buildLine — 3month carry-forward bucketing", () => {
  ...
  it("leading buckets before the first-ever log stay null", () => {
    const points = [{ date: "2026-06-27", value: 80 }]; // within bucket 3
  ...
  it("bucket value is the most recent point at/before bucket end, never averaged", () => {
    const points = [
      { date: "2026-06-17", value: 80.0 },
      { date: "2026-07-02", value: 79.0 },
    ];

    const line = buildLine("3month", points, "2026-04-08", "2026-07-06");

    // both points fall in bucket 3 (06-07..07-06)
    expect(line.points[2].value).toBe(79.0);
  });
