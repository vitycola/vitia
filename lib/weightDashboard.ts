/**
 * Pure data-shaping functions for the Progress > Peso dashboard.
 ...
 * lib/measurementsDashboard.ts, parameterized to a single hardcoded
 * metric (weightKg) instead of a MetricKey union: sparse (logged-days-only)
...
/**
 * Map ProgressEntry-like rows to sparse WeightPoint[], keeping ONLY rows
 * where weightKg is non-null. Ascending by date.
 */
export function toPoints(rows: WeightEntryRow[]): WeightPoint[] {
  return rows
    .filter((row) => row.weightKg != null)
    .map((row) => ({ date: row.date, value: row.weightKg as number }))
 ...
 * value of the most recent logged WeightPoint whose date <= bucket_i.to,
 ...
 */
export function deltaFromFirst(points: WeightPoint[]): number | null {
