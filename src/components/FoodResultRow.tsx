...
export function FoodResultRow({ food, onSelect }: FoodResultRowProps) {
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-xl bg-white px-4 py-3 text-left shadow-sm transition-colors hover:bg-surface active:bg-surface"
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="truncate text-sm font-medium text-primary">{food.name}</span>
          {food.hasMissingData && <AlertTriangle size={14} className="shrink-0 text-amber-500" />}
        </div>
        {food.brand && <p className="truncate text-xs text-disabled">{food.brand}</p>}
      </div>
