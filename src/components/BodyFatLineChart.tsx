import { windowSpanDays } from "@/lib/bodyFatDashboard";
import type { LinePoint, RenderState } from "@/lib/bodyFatDashboard";
...
interface BodyFatLineChartProps {
  points: LinePoint[];
...
/**
 * Hand-rolled SVG line chart (no charting library) — mirrors
 * WeightLineChart's no-library approach, but draws a line (not bars)
 * connecting only real logged body-fat % points, with no goal line. Three
 * explicit render states: "empty" (no data), "single" (one dot, no line),
 * "line" (polyline connecting >=2 real points).
 *
 ...
 * implicit in the slope, not annotated). 3-month bucket points
 * (LinePoint.key is "bucket-{index}") are spaced by index, which is already
 * correct/equivalent since each bucket is a uniform 30-day span by
 * construction.
 */
export function BodyFatLineChart({ points, renderState, window }: BodyFatLineChartProps) {
  ...
  return (
    <div className="relative" style={{ height: CHART_HEIGHT_PX }}>
      <svg
        ...
        role="img"
        aria-label="Gráfico de % grasa"
      >
        {renderState === "line" && (
          <polyline
            data-testid="bodyfat-line"
